...
if upload_file is not None and ferramenta != '':
    ...
    for coluna in df.iloc[:, [6, 8]]: # pode tirar isso
        if dados_filtrados.empty:
            st.warning("Não há dados para o dia selecionado.")
        else:
            ...
            with col1:
                with st.container():
                    ...
                    mes1 = st.selectbox(f"Mês1 para {coluna}:", df["Month"].unique())
                    dia1 = st.selectbox(f"Dia1 para {coluna}:", sorted(df["Day"].unique()))

                    filtro1 = df[(df["Month"] == mes1) & (df["Day"] == dia1)]

                    hora_inicial = st.selectbox(f'Selecione o horário inicial para {coluna}:', lista_horas)
                    
                    mes2 = st.selectbox(f"Mês2 para {coluna}:", df["Month"].unique())
                    dia2 = st.selectbox(f"Dia2 para {coluna}:", sorted(df["Day"].unique()))
                    ...
                    if not linha_inicial.empty and not linha_final.empty:
                        ...
                        if indice_final < indice_inicial:
                            st.warning("O índice final é menor que o índice inicial. Verifique os valores de entrada.")
